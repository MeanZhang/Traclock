...
import com.mean.traclock.backup.model.RestoreError
...
class BackupRestoreViewModel(
    ...
    fun backup(file: PlatformFile) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            var completed = 0
            file.sink().buffered().use { bufferedSink ->
                bufferedSink.writeString("Version: $BACKUP_FILE_VERSION\n")
                bufferedSink.writeString("Project ID, Project Name, Project Color\n")
                for (project in projects) {
                    bufferedSink.writeString("${project.id}, ${project.name}, ${project.color.toArgb()}\n")
                    setProgress(completed++.toFloat() / size)
                }
                bufferedSink.writeString(TABLE_SEPARATOR + "\n")
                bufferedSink.writeString("Record ID, Project ID, Start Time, End Time\n")
    ...
    fun restore() {
        ...
        restoreFile?.let { file ->
            viewModelScope.launch(Dispatchers.IO) {
                ...
                file.source().buffered().use { bufferedSource ->
                    var line = bufferedSource.readLine()
                    if (line == null) {
                        _restoreState.value = RestoreState.NO_DATA
                        return@launch
                    }
